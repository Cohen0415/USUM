...
镜像烧录流程如下：先把镜像文件从存储设备加载到内存，然后写入到eMMC中。所以这里需要确认内存地址。

uboot命令行执行`bdinfo`查看内存地址：
...
TLB addr    = 0xEFFF0000
relocaddr   = 0xEDC07000
reloc off   = 0xED207000
irq_sp      = 0xEB7F9070
sp start    = 0xEB7F9070
FB base     = 0x00000000
Early malloc usage: 3818 / 80000
fdt_blob = 000000000a100000
```

重点看两个DRAM bank。DRAM bank描述的是物理内存总容量，在选取地址前需要检查是否会和其它文件地址冲突。例如这里选取为`0x20000000`，将地址填入`USUM/u-boot/drivers/usum/cmd_usum.h`中：
